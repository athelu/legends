...
When you and an ally are on opposite sides of an enemy (with both in melee reach), both you and your ally subtract 1 from both dice when making attacks. The enemy must be no more than one size larger than you.

## The Dying Mechanic
A player character gains the dying condition whenever any attack reduces the character to 0 HP. Any damage from this attack that would take the character below 0 is ignored.The character immediately gains the following conditions:
- Dying
- Unconscious
- Blinded
- Prone

Non Player characters that are reduced to 0 HP (or below) are immediately killed, bypassing the dying state. 

### Stabilization
A character that gains the dying condition is at risk of being killed. The character can attempt to stabilize, which stops the dying condition from progressing. At the beginning of a player characters turn, while they have the dying condition, they need to make a fortitude save to attempt to stabilize.
- **0 successes:** Drop to -1 HP.
- **1+ successes:** Stabilize at 0 HP.
- Remove dying condition.
- Remain Unconscious, Blinded and Prone.
- Stabilization Recovery.

Once a character has successfully stabilized, they begin to make recovery tests at the beginning of their next turn. The character makes a will save to attempt to recover.
...
### Death Threshold
If the character begins their turn and their negative HP equals their constitution score they have been killed.

#### Example
- Constitution 4 character is killed at -4 HP.

**Ally Assistance**
An ally can use an [Interact] action to attempt to stabilize a character with the dying condition by making a Medicine skill test.
- **1 success:** You gain -1 to both dice on your next stabilization save.
- **2 successes:** You stabilize at 0 HP.

### Magical healing
A character that has the dying condition can be stabilized and immediately restored by the application of magical healing. If magically healed, the dying character removes the temporary conditions listed here:
- Dying
- Unconscious
- Blinded

## Death and Resurrection
Pneuma does not vanish instantly at death, it travels toward the nearest ley line or nexus before crossing into the outer planes. This creates a small window to attempt to recall the pneuma to the deceased individual.

There are limits however. The pneuma may be recalled within 24 hours from the moment of death.  If the death was caused by extreme destruction (Disintegrate, being consumed by fire, catastrophic violence) the ritual cannot be performed as it requires an intact enough corpse to serve as the anchor for returning pneuma.
If the death was the result of divine judgment or Nevil's deliberate claim then the pneuma is already beyond reach before the body is cold.
Attempts made beyond this time window fail, as the pneuma has crossed over to the outer planes. No mortal magic can reach it. The body can still be raised as undead through necromantic means, but the person is gone.

### Willingness
The departing soul must be willing to return. This maybe involve a discussion between the GM and the player. Things that should be taken into consideration are the creature's personality, attachments, and if they had any unfinished business; whether they died in peace or in desperate circumstances; whether their faith forbids resurrection (some interpretations of Nevil-worship hold that to flee death is to insult the god).
A soul that is unwilling causes the ritual to fail cleanly with no complications, just failure. The caster knows the soul declined.

### Consequences of Returning
Crossing back from the threshold leaves marks. All resurrected creatures suffer:
**Permanent HP reduction:** Maximum HP reduced by Constitution × 1 permanently. This can be offset by raising Constitution with XP but the scar remains in the fiction.

**The Pale:** For one week after resurrection, the creature adds 1 to both dice on all rolls involving Luck (saving throws, spending Luck).

**Dreams of the Crossing:** At GM discretion, the resurrected character receives fragmented visions or knowledge from their time between death and return. These should be narratively significant, not mechanically useful. Glimpses of the outer planes, impressions of where their pneuma was bound, or perhaps awareness of something watching.

**One Resurrection Only:** A character can be returned from death once. The second death is final. The crossing weakens the thread between body and pneuma. GMs may waive this for legendary circumstances, but it should feel like a story event, not a routine.
...
The characters will likely adventure and explore withing areas of varying levels of light. it is necessary to understand how light affects game play mechanics. Light function in levels, moving from the brightest to the darkest. There are five levels of light in Legends.

### Magical Radiant Light
Created by weaves (Radiant Sphere, Floating Lights, etc.) or divine effects. Treated as Bright Light but with additional effects.
**Effect:** Bright Light within radius, plus: Creatures with Light Sensitive flaw are Dazzled while within the radius. Creatures specifically vulnerable to sunlight are vulnerable within the light's radius.

### Bright Light
This is the default level of illumination. If a specific lighting condition is not stated, it should be assumed that the environment for the characters has Bright Light. Bright light encompasses normal daylight, torchlight within the initial radius, well lit room with windows. 
**Effect:** No visibility penalties. All vision functions normally.

### Dim Light
This is a decreased level of light, often found inside of buildings at night. Examples of this level of light are the ambient outdoor lighting at twilight, candlelight, the light of a torch at its outside radius, and moonlight.
**Effect:** Creatures in dim light gain the [Concealed] condition to attackers without Low-Light Vision or better vision. Vision-based Perception checks add 1 to both dice.
...
**Effect:** Same as Darkness, plus: Darkvision does not function inside magical darkness unless it is explicitly superior. Light weaves of equal or lesser Energy cost are suppressed 1 light effect level within the area. A Light weave of greater Energy cost than the darkness weave dispels it (GM adjudicates contested Energy costs).

### Light Sources
A character carrying a light source (torch, lantern, etc.) increases the level of light within an area specified by the item.
**Effect:** Provides Bright Light within the listed radius. Creatures outside the light radius can see the carrier from up to 4 x the bright light radius, removing the hidden/concealed conditions of the carrier provided by darkness (if applicable).


## Mounted Combat
When a character mounts a creature, the mount acts on the rider's initiative and does not have its own turn unless the rider is incapacitated or the mount is controlled by another. The rider spends their own actions to direct the mount's movement and special actions.

### Mount Actions
A mount can take the following actions at the rider's direction, spending the rider's actions:

#### Advance [Move]
The mount moves up to its speed. This uses one of the rider's actions.

#### Mount Attack [Combat]
The mount makes one natural weapon attack using its own attributes and combat skill. This uses one of the rider's [Combat] actions and is subject to the Multiple Action Penalty along with any other [Combat] actions the rider takes on the same turn.

#### Dash [Move]
The mount moves up to double its speed. Uses two of the rider's [Move] actions.

A rider may use their remaining actions for their own attacks, weaves, and other activities on the same turn.

### Untrained and Warhorse Mounts
Not all mounts are battle-trained. When a mount that is not combat-trained (GM's determination) is subjected to a [Frightened] condition, or when it takes damage while carrying a rider, the mount must make a Fortitude save (Con + Luck, 1 success). On failure, the mount bolts: it moves its full speed in a random direction on the rider's next turn and the rider has no directional control until the start of their following turn. A rider may attempt a Athletics check as a [Reaction] to keep control; success negates the bolt.

### Riding Checks
Riding is not a separate skill and instead falls under Athletics. Routine riding requires no check. Riding checks are called for when:
- Controlling a panicking mount (1 success)
- Executing difficult maneuvers at speed (GM discretion, 1-2 successes)
- Staying mounted when the mount is struck (1 success)
- Guiding a mount through difficult terrain while in combat (1 success)

### Falling from a Mount
When a rider fails a check to stay mounted, or is knocked prone while riding, they fall. A fall from a mount of standard size (Large) deals 4 bludgeoning damage and imposes the Prone condition.

### Targeting Riders and Mounts
The rider and mount together are treated as a single unit for targeting and positioning purposes. Attackers must choose to target either the rider or the mount specifically. Attacking a mounted rider adds 1 to both attack dice.

Mounts use standard bestiary stat blocks where available. Working animals used as mounts (horses, mules, camels) are TR 1/4 Standard creatures with the Beast type and no combat skills unless specified as warhorses or combat-trained.

## Aerial Combat
Aerial combat uses the same action economy and opposed roll mechanics as ground combat, with the following additions governing three-dimensional positioning and the challenges of fighting in the air.

### Sustained Flight vs. Hovering
Not all flying creatures are equal in the air. Creatures with a fly speed fall into one of two categories, noted in their stat block or ability description:

#### Sustained Flight.
The creature must keep moving to remain aloft. Birds, winged mounts, and most natural fliers fall into this category. A creature with Sustained Flight must spend one [Move] action per turn and move at least half its fly speed in distance. If it cannot or does not move this minimum distance, it loses altitude at the end of its turn, dropping one altitude band involuntarily. If this drop results in making ground contact, the creature takes 4 bludgeoning damage.
A Sustained Flight creature that wants to move less than half its fly speed on a turn must change altitude to compensate. For every 10 feet less than the minimum it travels horizontally, it must move up or down one altitude band. 

#### Hover
The creature can remain stationary in the air without penalty. Dragons, creatures with magical levitation, and those whose stat block explicitly notes hovering ability fall into this category. Hovering creatures have no minimum movement requirement and may remain at a fixed altitude indefinitely.

### Momentum Defense
A flying creature that has moved up to at least half its fly speed this turn gains the following benefit. Attacks against it add 1 to both attack dice. This benefit applies until the start of the creature's next turn and is lost if the creature is forced to stop moving (Grappled, Restrained, Stunned, Paralyzed, or any condition that reduces speed to 0).

### Altitude
Altitude is tracked in broad bands rather than precise feet:
...
Moving between altitude bands is done as part of the Advance action. Moving up one band costs an additional 10 feet of movement from the creature's fly speed. Dropping one band  may be done freely.

#### Losing Altitude
The following circumstances cause a flying creature to lose altitude involuntarily:
- **Failing the minimum movement requirement (Sustained Flight only):** Drop one band at end of turn, 4 bludgeoning damage on ground contact.
- **Stunned, Paralyzed, or Unconscious:** The creature falls immediately, dropping one altitude band per round until it hits the ground. It takes 8 bludgeoning damage per band fallen. Full DR applies.
- **Speed reduced to 0:** As Stunned above.

### Aerial Attacks
Melee attacks between airborne combatants require both to be in the same altitude band and within normal reach. A creature at Low altitude can make melee attacks against ground targets.

Ranged attacks against aerial targets follow standard range rules, treating altitude as part of the distance calculation. Add the lowest height of the targeted band to the horizontal distance away to determine the total range.

#### Diving Charge
A flying creature that moves at least 20 feet and descends to the low altitude band before making a melee attack on the same turn may declare a Diving Charge. The attack subtracts 1 from both attack dice. On a hit at Margin 2, add the creature's Strength to damage as normal plus an additional 4 damage from impact force. A Diving Charge counts toward the Sustained Flight minimum movement requirement for the turn.

#### Grappling in the Air
A flying creature that is successfully grappled is immediately dragged one altitude band downward if the grappler is at a lower altitude or on the ground. Each round that it remains grappled it continues to drop an altitude band taking 4 bludgeoning on ground contact. A Hovering creature that is grappled simply stops hovering freely but does not fall.

#### Prone
Prone has no meaningful effect on flying creatures. The prone condition is removed at the start of their next turn at no action cost, as they simply reorient in the air.
...
#### Movement
Creatures without a swim speed have their effective speed halved.

#### Melee Attacks
Melee attacks with weapons that deal Slashing or Bludgeoning damage add 1 to both attack dice. Piercing weapons. are unaffected and attack normally.

#### Ranged Attacks
Ranged weapon attacks add 2 to both attack dice, and can only be used at short range. Thrown weapons function at half their normal range.

#### Weaves
Fire-based weaves add 1 to both the weaving roll and targeting dice underwater. Fire damage dealt underwater is reduced by half.
Air-based weaves that create or manipulate breath or sound function normally. Any creature within 10 feet of the primary target  of  a Lightning-based weave  takes half the lightning damage with no save if they are in contact with the same body of water.
All other weaves function normally unless the GM determines the effect is physically nonsensical underwater.

#### Visibility
Clear water provides normal visibility to a range of 60 feet. Murky, dark, or disturbed water reduces visibility to 10 feet, imposing the Concealed condition on all creatures beyond that range. Total darkness underwater (deep water, no light source) imposes the Hidden condition on all creatures.

### Aquatic Creatures and Combat
Creatures with a designated swim speed do not suffer the standard underwater combat penalties. They attack and move normally and treat the environment as their native terrain.
Aquatic creatures without a walk speed are subject to the reverse on dry land: they move at half speed, and add 1 to both attack dice as they struggle with gravity and the absence of buoyancy.
